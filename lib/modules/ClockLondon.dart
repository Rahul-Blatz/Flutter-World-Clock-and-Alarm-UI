...
class ClockLondon extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return ClockLondonState();
  }
}

int londonHours, londonMinutes, londonSeconds;

class ClockLondonState extends State<ClockLondon> {
  //Method for getting London Time(Uses JSON to get London time data from World api) and that's why we use async

  void initState() {
    Timer.periodic(Duration(seconds: 1), (Timer t) => getLondonTime());
    super.initState();
  }
  ...
  void getLondonTime() async {
    ...
    londonHours = int.parse(londonTimeString.substring(11, 13));
    londonMinutes = int.parse(londonTimeString.substring(14, 16));
    londonSeconds = int.parse(londonTimeString.substring(17, 19));
  }
...
class ShapesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double secondsDegree = 360 / 60 * londonSeconds;
    ...
    double minutesDegree = 360 / 60 * londonMinutes;
    x = (size.width / 1) +
    ...
    double hoursDegree = 360 / 12 * (londonHours - 12);
    x = (size.width / 2) +
